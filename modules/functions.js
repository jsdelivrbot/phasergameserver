...
var fn = {
	...
	combindOver: function(obj, obj2) {
		//obj2 goes over obj, values that are not in obj are added
		for (var val in obj2) {
	...
	combindIn: function(obj, obj2) {
		//obj2 gose in obj, values that are not in obj are NOT added (works with arrays)
		for (var i in obj2) {
			if (obj[i] != undefined) {
	...
	applyDiff: function(obj, diff) {
		//applies the diff obj created by fn.getDiff
		diff = diff || {
			added: {},
			changed: {},
			removed: {}
	...
	buildDiff: function(diff) {
		//build an obj that represents the a diff obj
		diff = diff || {
			added: {},
			changed: {},
			removed: {}
