...
var maps = {
	...
	*/
	...
	Map: function(id) {
		...
		this.loadChunk = function(x, y, cb) {
			//creates a new chunk and tries to load it from the db
		...
		this.insertChunk = function(chunk, cb) {
			...
			db.query(
				"INSERT INTO `chunks`(`map`, `x`, `y`, `data`) VALUES(" +
					db.ec(data.map) +
					", " +
					db.ec(data.x) +
					", " +
		...
		this.saveChunk = function(x, y, cb) {
			//saves chunk to db (chunk has to exists in db)
			this.getChunk(
				x,
				y,
				function(chunk) {
		...
		this.unloadChunk = function(x, y, cb) {
			//saves and removes chunk from array
			this.saveChunk(
				x,
				y,
				function() {
	...
	Chunk: function(x, y, map) {
		...
		this.layerExists = function(i) {
			//tests to see if this layer is on the map
	...
	getMapList: function(cb) {
		//returns a list of maps not sorted by id
		db.query(
			"SELECT * FROM maps",
			function(data) {
				for (var i = 0; i < data.length; i++) {
	...
	getMap: function(mapID, cb) {
		//gets a map from the array, creates one if it dose not exists and it exists in the db. needs cb
	...
	setTiles: function(data, cb, dontFire) {
		//set tiles in a rect
		data = fn.combindOver(
			{
				x: 0,
	...
	loadMap: function(mapID, cb) {
		//returns a new map and trys to load it from db, if it wes in the db it puts it into the array
		map = new maps.Map(mapID);
		this.maps.push(map);
		//load it
		db.query(
			"SELECT * FROM maps WHERE id=" + db.ec(mapID),
			function(map, data) {
				map.loaded = true;
				if (data.length) {
	...
	saveMap: function(mapID, cb) {
		//saves a map from the array to the db (it has to already exist in the db)
		this.getMap(
			mapID,
			function(map) {
				data = map.exportData();
				...
				if (data.length) {
					db.query(
						"UPDATE `tiles` SET `blank`=" +
							db.ec(tile.blank) +
							", `collision`=" +
							db.ec(tile.collision) +
							", `collisionInfo`=" +
							db.ec(tile.collisionInfo) +
							" WHERE id=" +
							db.ec(tile.id),
						function() {
							if (cb) cb();
					...
					db.query(
						"INSERT INTO tiles(id,blank,collision,collisionInfo) VALUES(" +
							db.ec(tile.id) +
							", " +
							db.ec(tile.blank) +
							", " +
	...
	unloadMap: function(mapID, cb) {
		//saves and removes map from array
		this.saveMap(
			mapID,
			function() {
	...
	//db
	insertMap: function(map, cb) {
		//inserts a new map into the db based off an existing one
		data = map.exportData();
		db.query(
			"INSERT INTO `maps`(`name`, `width`, `height`, `desc`) VALUES(" +
				db.ec(data.name) +
				"," +
				db.ec(data.width) +
				"," +
	...
	deleteMap: function(mapID, cb) {
		...
		db.query(
			"DELETE FROM `maps` WHERE id=" + db.ec(mapID),
			function(data) {
	...
	createLayer: function(data, cb) {
		db.query(
			"INSERT INTO `layers`(`title`,`description`,`level`,`abovePlayer`,`visibleInGame`,`collision`) VALUES(" +
				db.ec(data.title) +
				"," +
				db.ec(data.description) +
				"," +
				db.ec(data.level) +
				"," +
				db.ec(data.abovePlayer) +
				"," +
				db.ec(data.visibleInGame) +
				"," +
				db.ec(data.collision) +
				")",
			function() {
	...
	saveMapLoop: function(i) {
		//repeating loop that saves maps
		if (this.maps[i]) {
			m = this.maps[i];
			if (m.saved == false) {
				this.saveMap(
	...
	unloadMapLoop: function(i) {
		//repeating loop that unloads maps when thet have not been gotten in 30min
		if (this.maps[i]) {
			var m = this.maps[i];
			var d = new Date();
