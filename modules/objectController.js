...
var objectController = {
	...
	*/
	...
	createObject: function(type, data, cb) {
		//creates an obj and inserts it into the db
		...
		var sql =
			"INSERT INTO `object-" +
			obj.type +
			"`(`x`, `y`, `map`, `width`, `height`, `properties`) VALUES (" +
			db.ec(obj.x) +
			"," +
			db.ec(obj.y) +
			"," +
	...
	deleteObject: function(id, type, cb) {
		//remove obj from db
		this.typeExists(
			type,
			function(exists) {
	...
	objectLoaded: function(id, type) {
		//checks to see if the obj is loaded
		return this.objects.indexOf({ id: id, type: type }) !== -1;
	},
	updateObject: function(id, type, data) {
		//updates objs data and fires event
		this.getObject(
			id,
			type,
			function(obj) {
	...
	typeExists: function(type, cb) {
		//checks to see if a type of object exists in the db
