...
var resources = {
	...
	resourceLoaded: function(data) {
		//handles events from objectController when a obj is loaded
	},
	...
	loadMap: function(mapID, cb) {
		...
		db.query(
			"SELECT url FROM maps WHERE id=" + mapID,
			function(data) {
				if (data.length) {
					//read the files
					fs.readFile(
	...
	resource: {
		//a obj for handling resources
		add: function(json, map) {
			//take a json obj from a map file and turns it into a resource obj
			a = [];
